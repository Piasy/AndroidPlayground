...
JNIEXPORT void JNICALL
        Java_piasy_github_com_voiceprocessing_VoiceProcessor_process(JNIEnv *env, jclass type,
                                                                     jbyteArray in_, jbyteArray out_,
                                                                     jint size, jfloatArray floatInput_,
...
Java_piasy_github_com_voiceprocessing_VoiceProcessor_process(JNIEnv *env, jclass type,
    ...
    smbPitchShift(2, 1024, 1024, 4, 44100, floatInput, floatOutput);
    for (i = 0; i < size; i += 2) {
        int frame = (int) floatOutput[i >> 1];
        out[i] = (jbyte) (frame & 0x00FF);
